...
/// Ported from the W2 monitor, which hit the problem first.
///
...
/// the moment the numbers hold still and the suffix carries the difference: <c>1.0.0-beta</c>,
/// <c>1.0.0-beta2</c> and <c>1.0.0</c> all truncate to <c>1.0.0</c>, so nobody on the first beta would
/// ever be offered the second or the final release. The app would tell them they were current,
/// indefinitely.
///
/// Two rules do most of the work, and both are easy to get wrong:
///   - A pre-release ranks **below** the same version without one: 1.0.0-beta &lt; 1.0.0.
